...
/* alan add SOCK_CLOEXEC for socket fork-safe*/
...
/* alan add for tcp-delay optimization*/
...
/* alan set the concurrency authen-link to 32*/
    listen(sock,32);
...
httpdReadRequest(httpd * server, request * r)
{
     ...
     */
    strcpy(r->response.headers, "Server: RuiWei Technologies Embedded Server\n");
