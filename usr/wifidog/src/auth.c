...
logout_client(t_client * client)
{
    ...
    wa_client_audit(ACT_OFFLINE_AUDIOT,client,1);
    fw_deny(client);
...
authenticate_client(request * r)
{
    ...
    /*alan modified: 
    * 1. delete client from list when authen failed!
    * 2. cut back the client-list LOCK!
    */
    case AUTH_ERROR:
    ...
    case AUTH_DENIED:
        ...
        wa_client_audit(ACT_OFFLINE_AUDIOT,tmp,1);
        fw_deny(client);
        client_list_delete(client);
        UNLOCK_CLIENT_LIST();
    ...
    case AUTH_ALLOWED:
        ...
        wa_client_audit(ACT_ONLINE_AUDIOT,client,1);
        served_this_session++;
        UNLOCK_CLIENT_LIST();
