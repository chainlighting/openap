...
static const struct {
    ...
    "authouispecial",oAuthServOuiSpecial},{
    ...
    "wifiaudiot", oWifiAudiot}, {
    "waenable", oWifiAudiotEnable}, {
    "wacheckinterval", oWACheckInterval}, {
    "waprobeenable", oWAProbeEnable}, {
    "waproberetries", oWAProbeRetries}, {
    "waprobewaittime", oWAProbeWaitTime},{
    "waserver",oWAServer}, {
    "waserverhostname",oWAServerHostName},{
    "waserverport",oWAServerPort},{
NULL, oBadOption},};
...
config_init(void)
{
    ...
    config.wa_servers = NULL;
    config.wa_enable = 0;
    config.wa_chk_interval = 5;
    config.wa_probe_enable = 0;
    config.wa_probe_req_retries = 5;
    config.wa_probe_req_waittime = 5;
...
parse_auth_server(FILE * file, const char *filename, int *linenum)
{
    ...
    authservouispecial = safe_strdup(DEFAULT_AUTHSERVOUISPECIAL);
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
            ...
            case oAuthServSSLAvailable:
                ...
                break;
            case oAuthServOuiSpecial:
                free(authservouispecial);
                authservouispecial = safe_strdup(p2);
                break;
            case oBadOption:
            default:
    ...
    new->authserv_oui_special = authservouispecial;
...
/** @internal
Parses wifi-audit setting
*/
static void
parse_wifi_audiot(FILE * file, const char *filename, int *linenum)
{
    ...
    while (memset(line, 0, MAX_BUF) && fgets(line, MAX_BUF - 1, file) && (strchr(line, '}') == NULL)) {
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
            ...
            switch (opcode) {
            case oWifiAudiotEnable:
                config.wa_enable = atoi(p2);
                break;
            case oWACheckInterval:
                config.wa_chk_interval = atoi(p2);
                break;
            case oWAProbeEnable:
                config.wa_probe_enable = atoi(p2);
                break;
            case oWAProbeRetries:
                config.wa_probe_req_retries = atoi(p2);
                break;
            case oWAProbeWaitTime:
                config.wa_probe_req_waittime = atoi(p2);
                break;
            default:
                debug(LOG_ERR, "Bad option on line %d " "in %s.", *linenum, filename);
                debug(LOG_ERR, "Exiting...");
    ...
    debug(LOG_DEBUG, "WifiAudiot Setting: en(%d),intval(%d),probeEn(%d),probeRetries(%d),probeWaitTime(%d)",
        config.wa_enable,config.wa_chk_interval,config.wa_probe_enable,
        config.wa_probe_req_retries,config.wa_probe_req_waittime);
}

/** @internal
Parses wifi-audit server information
*/
static void
parse_wa_server(FILE * file, const char *filename, int *linenum)
{
    char *host = NULL,
        *port = NULL,
        line[MAX_BUF], *p1, *p2;
    ...
    /* Defaults */
    host = safe_strdup(DEFAULT_WA_SERVER_HOSTNAME);
    port = safe_strdup(DEFAULT_WA_SERVER_PORT);
    ...
    debug(LOG_DEBUG, "Adding %s:%s to the wa server list", host, port);

    /* Allocate memory */
    new = safe_malloc(sizeof(t_wa_serv));
    ...
    /* If it's the first, add to config, else append to last server */
    if (config.wa_servers == NULL) {
        config.wa_servers = new;
    } else {
        for (tmp = config.wa_servers; tmp->next != NULL; tmp = tmp->next) ;
        tmp->next = new;
    }

    debug(LOG_DEBUG, "wifi-audit server added");
}
        ...
        if (p1 && p1[0] != '\0') {
            ...
            if ((strncmp(s, "#", 1)) != 0) {
                ...
                case oAuthServer:
                    ...
                    break;
                case oWifiAudiot:
                    parse_wifi_audiot(fd,filename,&linenum);
                    break;
                case oWAServer:
                    parse_wa_server(fd,filename,&linenum);
                    break;
