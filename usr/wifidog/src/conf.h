...
#define DEFAULT_AUTHSERVOUISPECIAL "NORMAL"
#define CMCC_AUTHSERVOUISPECIAL "CMCC"
/*@}*/

/** Note The wifi-audit server default setting */
#define DEFAULT_WA_SERVER_HOSTNAME "127.0.0.1"
#define DEFAULT_WA_SERVER_PORT     "8000"
/*@}*/
...
typedef struct _auth_serv_t {
    ...
    char *authserv_oui_special;       /**< @brief Use normal wifidog auth or oui-special auth*/
    char *last_ip;      /**< @brief Last ip used by authserver */
    struct _auth_serv_t *next;
} t_auth_serv;

/**
* wifi audit server info
*/
typedef struct _wa_serv_t {
    char* hostname;            /**< @brief hostname of the audit server*/
    char* port;                /**< @brief port of the audit server*/
    struct _wa_serv_t* next;
} t_wa_serv;
...
typedef struct {
    ...
    t_wa_serv* wa_servers;      /**< @brief wifi-audit server list*/
    int wa_enable;              /**< @brief wifi-audit enable*/
    int wa_chk_interval;        /**< @brief Frequence of the wa_audiot check interval*/
    int wa_probe_enable;        /**< @brief Enable the wifi probe request to find the arround site*/
    int wa_probe_req_retries;   /**< @brief the probe request retries times of one probe loop*/
    int wa_probe_req_waittime;  /**< @brief Frequence of the site survey probe interval*/
} s_config;
