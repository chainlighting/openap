...
static pthread_t tid_wifiaudit = 0;
...
termination_handler(int s)
{
    ...
    if(tid_wifiaudit && (self != tid_wifiaudit)) {
        debug(LOG_INFO, "Explicitly killing the wifiaudit thread");
        pthread_kill(tid_wifiaudit, SIGKILL);
    }
...
main_loop(void)
{
    ...
    /*Start wifi-audit thread*/
    result = pthread_create(&tid_wifiaudit,NULL,(void*)wa_thread_wifi_audit,NULL);
    if(result != 0) {
        debug(LOG_ERR, "FATAL: Failed to create a new thread (wifi-audit) - exiting");
        termination_handler(0);
    }
    pthread_detach(tid_wifiaudit);
