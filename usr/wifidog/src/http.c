    ...
    if (!is_online()) {
        ...
        if (!(mac = arp_get(r->clientAddr))) {
            ...
            debug(LOG_INFO, "Failed to retrieve MAC address for ip %s, so not putting in the login request",
                  r->clientAddr);
            if(0 == strcasecmp(auth_server->authserv_oui_special,CMCC_AUTHSERVOUISPECIAL)) {
                /*v=S2&umac=0C771A4D4269&uip=172.16.90.11&apmac=84742AB6327D&apip=172.16.10.18&radio=00&vap=00&apport=8000&rssi=-46*/
                safe_asprintf(&urlFragment, "%sv=S2&apip=%s&apport=%d&apmac=%s&uip=%s&uurl=%s&radio=00&vap=00&rssi=-46",
            ...
            if(0 == strcasecmp(auth_server->authserv_oui_special,CMCC_AUTHSERVOUISPECIAL)) {
                safe_asprintf(&urlFragment, "%sv=S2&apip=%s&apport=%d&apmac=%s&uip=%s&umac=%s&uurl=%s&radio=00&vap=00&rssi=-46",
